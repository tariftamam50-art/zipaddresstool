...
import { Search, MapPin, Globe, Home, Shield, Info, Copy, Check, Navigation, Database, Phone, Smartphone, RefreshCw, User } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const fetchGeoData = async (query: string = '') => {
    ...
    try {
      if (mode === 'ip') {
        ...
        // Automatically trigger data extraction for ZIP mode
        scanForAddresses(newGeoData);
      }
    } catch (err: any) {
  ...
  const scanForAddresses = async (data: GeoData) => {
    ...
    try {
      ...
      // --- STEP 3: Generate phone numbers ---
      setPhoneSearchStatus('Searching SMS receiving services...');
      const smsServices = [
        { name: 'receive-sms-free.cc', getUrl: (num: string) => `https://receive-sms-free.cc/Free-USA-Phone-Number/${num.replace(/[+\s()-]/g, '')}` },
        { name: 'temporarynumber.com', getUrl: (_num: string) => `https://temporarynumber.com/en/country/us` },
        { name: 'temp-number.com', getUrl: (_num: string) => `https://temp-number.com/temporary-numbers` },
        { name: 'receive-smss.com', getUrl: (num: string) => `https://receive-smss.com/sms/${num.replace(/[+\s()-]/g, '')}` },
        { name: 'sms-activate.org', getUrl: (_num: string) => `https://sms-activate.org/en/` },
      ];
      ...
      for (let i = 0; i < 5; i++) {
        ...
        phResults.push({ number: formatted, cleanNumber: clean, areaCode, source: service.name, url: service.getUrl(clean) });
      }
      setPhoneResults(phResults);

      // --- STEP 4: Generate Male Names ---
      setPhoneSearchStatus('Generating identities...');
      const maleFirstNames = ['James', 'Robert', 'John', 'Michael', 'David', 'William', 'Richard', 'Joseph', 'Thomas', 'Christopher', 'Charles', 'Daniel', 'Matthew', 'Anthony', 'Mark', 'Donald', 'Steven', 'Paul', 'Andrew', 'Joshua', 'Kenneth', 'Kevin', 'Brian', 'George', 'Timothy', 'Ronald', 'Edward', 'Jason', 'Jeffrey', 'Gary'];
      const lastNames = ['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia', 'Miller', 'Davis', 'Rodriguez', 'Martinez', 'Hernandez', 'Lopez', 'Gonzalez', 'Wilson', 'Anderson', 'Thomas', 'Taylor', 'Moore', 'Jackson', 'Martin', 'Lee', 'Perez', 'Thompson', 'White', 'Harris', 'Sanchez', 'Clark', 'Ramirez', 'Lewis', 'Robinson'];
      
      const identities: IdentityResult[] = [];
      for (let i = 0; i < 5; i++) {
        const first = maleFirstNames[Math.floor(Math.random() * maleFirstNames.length)];
        const last = lastNames[Math.floor(Math.random() * lastNames.length)];
        identities.push({ firstName: first, lastName: last });
      }
      setIdentityResults(identities);
              ...
              <div className="lg:col-span-2 space-y-6">
                {geoData.isZipSearch ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-full">
                    ...
                    {/* Phone & Identity Panels (Stacked Column) */}
                    <div className="flex flex-col space-y-6">
                      {/* Identity Panel */}
                      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col">
                        <div className="flex items-center justify-between mb-6">
                          <h3 className="text-lg font-semibold flex items-center">
                            <User className="w-5 h-5 mr-2 text-indigo-400" /> Male Identities
                          </h3>
                        </div>

                        {!scanning && identityResults.length === 0 && (
                          <div className="p-8 border-2 border-dashed border-slate-800 rounded-2xl bg-slate-900/50 text-center">
                            <p className="text-sm text-slate-500">Extract identities for {geoData.city}</p>
                          </div>
                        )}

                        {!scanning && identityResults.length > 0 && (
                          <div className="space-y-3">
                            {identityResults.map((id, idx) => (
                              <div key={idx} className="p-3 bg-slate-800/40 rounded-xl border border-slate-700/50 space-y-2">
                                <div className="flex justify-between items-center group">
                                  <div className="flex flex-col">
                                    <span className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">First Name</span>
                                    <span className="text-sm font-medium">{id.firstName}</span>
                                  </div>
                                  <button
                                    onClick={() => copyToClipboard(id.firstName, `fn-${idx}`)}
                                    className="p-2 bg-slate-900 rounded-lg hover:text-indigo-400 border border-slate-800 transition-colors"
                                  >
                                    {copiedPhone === `fn-${idx}` ? <Check className="w-4 h-4 text-green-400" /> : <Copy className="w-4 h-4" />}
                                  </button>
                                </div>
                                <div className="flex justify-between items-center group">
                                  <div className="flex flex-col">
                                    <span className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Last Name</span>
                                    <span className="text-sm font-medium">{id.lastName}</span>
                                  </div>
                                  <button
                                    onClick={() => copyToClipboard(id.lastName, `ln-${idx}`)}
                                    className="p-2 bg-slate-900 rounded-lg hover:text-indigo-400 border border-slate-800 transition-colors"
                                  >
                                    {copiedPhone === `ln-${idx}` ? <Check className="w-4 h-4 text-green-400" /> : <Copy className="w-4 h-4" />}
                                  </button>
                                </div>
                              </div>
                            ))}
