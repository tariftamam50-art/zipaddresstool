...
const App: React.FC = () => {
  ...
  const fetchGeoData = async (query: string = '') => {
    ...
    try {
      if (mode === 'ip') {
        // Fallback for ipapi.co which sometimes blocks direct browser requests
        const url = query ? `https://ipapi.co/${query}/json/` : `https://ipapi.co/json/`;
        const response = await axios.get(url).catch(() => {
          // Second fallback to another free provider if first fails
          return axios.get('https://ip-api.com/json/');
        });
        
        const data = response.data;
        // Normalize data from second provider if needed
                      ...
                      <div className="p-4 bg-slate-800/30 rounded-2xl border border-slate-700/50">
                        ...
                        <div className="flex items-center space-x-4">
                          <div className="flex flex-col">
                            ...
                            <span className="font-mono text-sm text-indigo-300">{geoData.latitude.toFixed(4)}</span>
                          </div>
                          <div className="w-px h-8 bg-slate-700"></div>
                          <div className="flex flex-col">
                            <span className="text-[10px] text-slate-500 uppercase tracking-tighter">Longitude</span>
                            <span className="font-mono text-sm text-indigo-300">{geoData.longitude.toFixed(4)}</span>
                          </div>
                        </div>
                      </div>
                    </div>
